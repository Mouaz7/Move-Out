...
describe('Profile Routes', () => {
  ...
  describe('Profile Update', () => {
    test('should verify current password correctly', async () => {
      const isValid = await bcrypt.compare('CurrentPassword123!', mockUser.password_hash);
      expect(isValid).toBe(true);
    });

    test('should reject incorrect current password', async () => {
      const isValid = await bcrypt.compare('WrongPassword123!', mockUser.password_hash);
  ...
  describe('Account Reactivation', () => {
    ...
    test('should reject reactivation for already active account', async () => {
      mockConnection.query.mockResolvedValue([[{ ...mockUser, is_active: true }]]);
